...
\begin{figure}[!htbp]
	\centering
	\includegraphics[height=60mm, ]{Figures/Theory/Pressure.pdf}
...
\begin{figure}[!htbp]
	...
	\caption{The schematics of control volume considered for the perfectly stirred reactor that encompasses the gas mixture and excludes the soot particles. Mass and energy are transferred between gas and soot particles. The inlet flow brings species and enthalpy into the control volume and the outlow discharges them. The outflow  mass flow is less inflow mass flow due to soot formation.}
	\label{fig:psrcv}
\end{figure}
...
\citet{blanquart2009joint} estimated the constant ${C_{N,j}}$ by comparing the profiles of several PAH species with experimental measurements in a single premixed benzene flame~\citep{tregrossi1999combustion}, and provided a efficiency values for various PAHs that are listed in Table~\ref{tab:gammalist_dimcoal}. The rate of dimer collision is expressed as:
